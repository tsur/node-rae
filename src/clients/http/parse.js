...
import { NoChallengeScriptFoundError } from "clients/http/errors";
...
function parseAuth(domAsString) {
  ...
  return function(resolve, reject) {
    try {
      ...
      const oohhMyRaeFriendthatsAnEasyChallengeHaHa = new Function(
        `return ${crc
  ...
  return function(resolve, reject) {
    try {
      ...
      if (!result.multipleMatches){
        ...
        result.items.push({match: $(elem).text(), id: $(elem).attr('href').replace(RAE_FETCH_ACTION, '')}));
      }
...
function challenge() {
  ...
  for (var i = 0; i < m - 1; i++) {
    ...
    for (var k = 0, iTop = str.length; k < iTop; k++) {
      crc =
        (crc >> 8) ^
        ("0x" + table.substr(((crc ^ str.charCodeAt(k)) & 0x000000ff) * 9, 8));
    }
    crc = crc ^ -1;
    crc = Math.abs(crc);
    if (crc == parseInt(c)) {
  ...
  document.forms[0].elements[1].value =
    "854f7d8b760998fb67664a923f839453:" + chlg + ":" + slt + ":" + crc;
  decode_action();
  if (send_cookie == 0) {
    submit_form();
  } else {
