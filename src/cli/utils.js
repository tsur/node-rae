...
export function shouldDisplayHelp(options){
  return (isEmpty(options._) && keys(options).length === 1) || 
...
function printResult(result) {
  ...
  result.items.forEach(({id}, i) => dictionary[String(i+1)] = id);
  result.items.forEach(({match}, i) => console.log(`\t${(i + 1)}. ${String(match).replace(/\./g,'')}`));
  ...
  const processLine = function(wordIndex) {
